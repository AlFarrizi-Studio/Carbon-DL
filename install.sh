...
if command -v curl >/dev/null 2>&1; then
    TAG_NAME=$(curl -fsSL "https://api.github.com/repos/$GITHUB_REPO/releases/latest" | grep -o '"tag_name": *"[^"]*"' | cut -d'"' -f4)
elif command -v wget >/dev/null 2>&1; then
    TAG_NAME=$(wget -qO- "https://api.github.com/repos/$GITHUB_REPO/releases/latest" | grep -o '"tag_name": *"[^"]*"' | cut -d'"' -f4)
else
    echo -e "${RED} ✗ Neither curl nor wget found. Cannot fetch release info.${RESET}"
    exit 1
fi

if [ -z "$TAG_NAME" ]; then
    echo -e "${RED} ✗ Failed to fetch latest release info.${RESET}"
...
if [ "$OS" = "Darwin" ]; then
    ASSET_NAME="carbon-dl-macos.tar.gz"
else
    ASSET_NAME="carbon-dl-linux.tar.gz"
fi
...
tar -xzf "$TMP_DIR/$ASSET_NAME" -C "$INSTALL_DIR/app"
